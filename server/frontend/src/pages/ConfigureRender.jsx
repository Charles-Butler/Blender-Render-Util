...
function ConfigureRender({ onStartRender, readOnly = false, onCancelRender, renderState }) {
  ...
  const fetchRenderState = async () => {
    try {
      ...
      if (data.status === 'ok' && data.render_state) {
        ...
        const allBatches = [
          ...(state.completed_list || []).map(b => ({ ...b, status: 'completed' })),
  ...
  const getSortedBatches = () => {
    return [...batches].sort((a, b) => {
      // Priority: 1 > 0 > null
      const priorityA = a.priority === '1' ? 3 : a.priority === '0' ? 2 : 1
      const priorityB = b.priority === '1' ? 3 : b.priority === '0' ? 2 : 1
  ...
  const getPriorityColor = (priority) => {
    ...
    if (priority === '0') return '#60a5fa' // blue
    return '#9ca3af' // gray
  }
  ...
  return (
    <div className="configure-render">
      <div className="configure-container">
        <h2>{readOnly ? 'Current Render Configuration' : 'Configure Batch Render'}</h2>
        ...
        <div className="config-grid">
          ...
          <div className="config-section">
            ...
            <div className="action-buttons-inline">
              {readOnly ? (
                <button className="btn-danger" onClick={onCancelRender}>
                  🛑 Cancel Render Job
                </button>
              ) : (
                <button
            ...
            <div className="section-header">
              ...
              {!readOnly && (
                <div className="frame-selection-buttons">
                  ...
                  <button
                    className="btn-icon"
                    onClick={() => batches.length > 0 && handleRemoveBatch(batches.length - 1)}
                    disabled={batches.length === 0}
            ...
            {showAddBatch && !readOnly && (
              <div className="add-batch-form">
                ...
                <div className="form-actions">
                  ...
                  <button className="btn-secondary" onClick={() => setShowAddBatch(false)}>
                    Cancel
                  </button>
                </div>
              </div>
                ...
                <div className="order-list">
                  {getSortedBatches().map((batch, idx) => (
                    <div key={idx} className="order-item">
                      ...
                      <span className="order-frames">({batch.frames} frames)</span>
                      <span
                        className="priority-badge small"
                        style={{ backgroundColor: getPriorityColor(batch.priority) }}
                      >
        ...
        <div className="config-section full-width-section">
          <h3>📋 Batch Queue ({batches.length} batches, {getTotalFrames()} frames)</h3>
          ...
          {batches.length === 0 && (
            <div className="empty-state">
              <p>No batches added yet. Click "+" in Frame Selection to get started.</p>
            </div>
          )}
        </div>
      </div>
